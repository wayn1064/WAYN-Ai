...
import { userStore, type ClinicUser } from '../store/userStore';
import { mockPubSub } from '../../../shared/utils/mockPubSub';

export const UserListPage: React.FC = () => {
  const [users, setUsers] = useState<ClinicUser[]>(userStore.approvedUsers);
  ...
  useEffect(() => {
    ...
    const unsubscribe = mockPubSub.subscribe('USER_APPROVED', (newUser: ClinicUser) => {
      // 이미 목록에 있는지 확인 후 추가
      setUsers((prev) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div>
        ...
        <p className="text-slate-500 mt-1">현재 DENTi-Ai 시스템에 가입되어 활성화된 직원 목록입니다.</p>
      </div>
